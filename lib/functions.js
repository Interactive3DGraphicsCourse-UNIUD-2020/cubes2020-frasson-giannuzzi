...
function Builder(first, data, cubebox) {
    ...
    for (i = 0; i < groundNumBoxX; i++) {
        for (j = 0; j < groundNumBoxZ; j++) {
            ...
            var c = Math.floor(map_range(data[k], Math.min.apply(Math, data), Math.max.apply(Math, data), 0, 255));
            c = (c.toString(16).length < 2 ? "0" : "") + c.toString(16);
            cube.material.color.setHex("0X" + c + "" + c + "" + c);
            
            if(rainbow){
                var c = Math.floor(map_range(data[k], Math.min.apply(Math, data), Math.max.apply(Math, data), 0, 365));
                var color = new THREE.Color("hsl("+ c +", 100%, 50%)");
            ...
            if (first) {
                ...
                if(rainbow){
                    cubebox.children[k].material.color = color;
                }else{
